...
export function GoogleVideosRadarPanel() {
  ...
  const refreshStatus = useCallback(async (): Promise<{ retryable?: boolean } | null> => {
    ...
    if (!res.ok) {
      if (j.retryable) return { retryable: true }
  ...
  const carregar = useCallback(async () => {
    ...
    let instavel = false
    try {
      const [actorsRes, mentionsRes, statusResult] = await Promise.all([
      ...
      if (statusResult?.retryable) instavel = true
