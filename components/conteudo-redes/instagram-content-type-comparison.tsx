      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-background">
        <div
          className={cn(
            ...
            isLeader ? 'bg-status-success' : 'bg-accent-gold/55'
          )}
        ...
        {CONTENT_TYPE_ORDER.map((type) => {
              ...
              <div key={type} className={cn(panelClassName, 'opacity-70')}>
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-secondary">Sem postagens no período</p>
                  </div>
                </div>
              </div>
            )
          ...
          return (
            <div
              ...
              className={cn(
                panelClassName,
                isOverallLeader && 'ring-2 ring-[rgb(var(--color-primary)/0.25)]',
                config.accentClass
              )}
            >
              <div className="mb-4">
                <div className="mb-2 flex items-start justify-between gap-2">
                  ...
                  {isOverallLeader && (
                    <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-[rgb(var(--color-primary-tint))] px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-[rgb(var(--color-primary))]">
                      <Crown className="h-3 w-3" />
                      Destaque geral
                    </span>
                  )}
                </div>
                <p className="text-sm text-secondary">{stats.posts} postagens</p>
                {strengthLabel && (
                  <p className="mt-2 text-xs font-medium text-text-primary">{strengthLabel}</p>
                )}
                {highlights.length > 0 && (
              ...
              <div>
                <p className="mb-2 text-xs text-secondary">Média por postagem</p>
