...
export async function GET(request: Request) {
  ...
  type AccLider = {
    nome: string
    handles: Set<string>
    /** Nome do liderado (`leads_militancia.nome`) por @ normalizado. */
  ...
  type AccLideradoEng = {
    comentarios: number
    medias: Set<string>
    /** ISO do comentário mais recente usado para escolher `ultimaPublicacaoComentadaEm`. */
    ultimoCommentedAt: string | null
    /** `media_posted_at` da linha do comentário mais recente (fallback: `commented_at`). */
    ultimaPublicacaoComentadaEm: string | null
  }
  ...
  for (;;) {
    ...
    for (const row of rows) {
      ...
      const reg = tdTerritorialPorLeadRowLike(row)
      if (reg !== td) continue
    ...
    for (const row of rows) {
      ...
      if (commentedAt) {
        if (!eng.ultimoCommentedAt || commentedAt > eng.ultimoCommentedAt) {
          ...
          eng.ultimaPublicacaoComentadaEm =
            mediaPostedAt || commentedAt || null
        }
      }
    }
