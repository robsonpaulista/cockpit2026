      ...
      <div className="px-4 py-6 lg:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {relatorios.map((relatorio, idx) => (
            <div
              key={idx}
              className="bg-surface rounded-2xl border border-card p-6 hover:shadow-card-hover transition-all duration-200 ease-out"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="p-3 rounded-lg bg-accent-gold-soft">
                  <FileBarChart className="w-5 h-5 text-accent-gold" />
                </div>
                <button className="p-2 rounded-lg hover:bg-background transition-colors">
                  <Download className="w-4 h-4 text-secondary" />
                </button>
              </div>
              <h3 className="text-lg font-semibold text-primary mb-2">{relatorio.tipo}</h3>
              <p className="text-sm text-secondary mb-4">{relatorio.descricao}</p>
